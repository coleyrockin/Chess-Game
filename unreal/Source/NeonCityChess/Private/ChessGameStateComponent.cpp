...
bool UChessGameStateComponent::RefreshState(FString& OutError)
{
    ...
    {
        OutError = TEXT("ExportScriptPath is empty. Set it to unreal/export_state.py.");
        return false;
    }

    const FString PythonExe = PythonExecutable.FilePath.IsEmpty() ? TEXT("python3") : PythonExecutable.FilePath;
    const FString ScriptPath = FPaths::ConvertRelativePathToFull(ExportScriptPath.FilePath);
    if (!FPaths::FileExists(ScriptPath))
    {
        OutError = FString::Printf(TEXT("Export script not found: %s"), *ScriptPath);
        return false;
    }
    ...
    {
        WorkingDir = FPaths::GetPath(ScriptPath);
    }
